...
vector<int> jimOrders(vector<vector<int>> orders) {
    vector<pair<int, int>> serveTimes; // pair of (serve_time, customer_id)

    for (int i = 0; i < orders.size(); i++) {
        int orderTime = orders[i][0];
        int prepTime = orders[i][1];
        int serveTime = orderTime + prepTime;
        serveTimes.push_back({serveTime, i + 1}); // customer IDs are 1-based
    }

    // Sort by serve time, and by customer ID if serve times are equal
    sort(serveTimes.begin(), serveTimes.end());

    // Extract customer order in which burgers are served
    vector<int> result;
    for (auto &p : serveTimes) {
